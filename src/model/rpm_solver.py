...
class RelNet(nn.Module):
    def __init__(self, model, n_s):
        ...
        if model in ["Rel-AIR", "Rel-Base"]:
            ...
            lin_in = O_OC * F_PL
            self.og_net = nn.Sequential(ResBlock(9 if model == "ResNet" else 8, O_HC, O_HC, 2),
                                        ResBlock(O_HC, O_HC, O_OC, 2))

        else:
            print("Model \"{}\" unrecognised.".format(model))
            sys.exit(1)

        if model in ["Rel-AIR", "Rel-Base", 'ResNet', 'Context-blind']:
...
class RPM_Solver(BasicModel):
    ...
    def compute_loss(self, output, target, reduction='mean'):
        # target: bs
        # output: bs x 8
    ...
    def forward(self, x, n_s, t=None):
        ...
        out = self.rel_enc(x, n_s, t)  # batch_size x 8
        return out
